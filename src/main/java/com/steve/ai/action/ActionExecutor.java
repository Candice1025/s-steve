...
public class ActionExecutor {
    ...
    public void processNaturalLanguageCommand(String command) {
        ...
        try {
            ...
            if (response == null) {
                SteveMod.LOGGER.error("❌ AI returned NULL response for command: {}", command);
            ...
            SteveMod.LOGGER.info("📋 Steve '{}' received {} tasks from AI", steve.getSteveName(), taskQueue.size());
            
            // Log each task for debugging
            int taskNum = 1;
            for (Task task : taskQueue) {
                SteveMod.LOGGER.info("  Task {}: action='{}', params={}", taskNum++, task.getAction(), task.getParameters());
            }
            ...
            SteveMod.LOGGER.error("❌ Error processing command: {}", command, e);
            sendToGUI(steve.getSteveName(), "Sorry, something went wrong!");
        }
